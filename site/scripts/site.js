...
var FilterMenu = function () {
    ...
    FilterMenu.prototype.dropdownFilterItem = function (td, self) {
        ...
        var value = td.getAttribute('data-find');
    ...
    FilterMenu.prototype.dropdownFilterContent = function () {
        ...
        var innerDivs = this.tds.reduce(function (arr, el) {
            var values = arr.map(function (el) {
                ...
                return el.getAttribute('data-find').trim();
            });
            if (values.indexOf(el.getAttribute('data-find').trim()) < 0) arr.push(el);
            return arr;
        }, []).sort(function (a, b) {
            // var A = a.innerText.toLowerCase();
            // var B = b.innerText.toLowerCase();
            var A = a.getAttribute('data-find').toLowerCase();
            var B = b.getAttribute('data-find').toLowerCase();
...
var FilterCollection = function () {
    function FilterCollection(target, options) {
        ...
        this.rows = target.find('tbody').find('tr[class="ajax-item"]').toArray();
    ...
    FilterCollection.prototype.updateRowVisibility = function (filterMenus, rows, ths) {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            for (var j = 0; j < selectedLists.length; j++) {
                var content = tds[selectedLists[j].column].getAttribute('data-find').trim().replace(/ +(?= )/g, '');
    ...
    FilterCollection.prototype.sort = function (column, order, table, options) {
        ...
        var rows = jQuery(tbody).find('tr[class="ajax-item"]').get();
        rows.sort(function (a, b) {
            // var A = a.children[column].innerText.toUpperCase();
            // var B = b.children[column].innerText.toUpperCase();
            var A = a.children[column].getAttribute('data-find').toUpperCase();
            var B = b.children[column].getAttribute('data-find').toUpperCase();
            if (!isNaN(Number(A)) && !isNaN(Number(B))) {
 ...
 (function($) {
	...
	Calendar.prototype = {
		...
		_renderDataSourceDay: function(elt, currentDate, events) {
			var currentRisk = 'day-' + events[0].risk,
			content = '',
			content2 = '<div class="circle-wrapper">';
			// console.log('$$$ ---------------------------- ');
			// console.log('$$$ elt', elt);
			...
			// var classCircle = events.length > 1 ? 'event-tooltip-wrapper-has-circle' : '';

			if(events.length > 0) {
				content += '<div class="event-tooltip-wrapper ">';
				for(var i in events) {
					var eventRisk = events[i].risk ? events[i].risk : '',
					eventCategory = events[i].location ? events[i].location : '',
					eventName = events[i].name ? events[i].name : '';

					content += '<div class="event-tooltip-content event-tooltip-content-' + eventRisk + '">'
	                                              + '<div class="event-name" style="color:' + events[i].color + '">' + eventCategory + '</div>'
	                                              + '<div class="event-location">' + eventName + '</div>'
	                                          + '</div>';
	            }
	            for(var i in events) {
	            	content2 += '<div class="circle" style="background-color:' + events[i].color + '"></div>'
	            }
	            content2 += '</div>';
	            content += '</div>';
	        }

			jQuery(elt).append(content); 
			if (events.length > 1)
				jQuery(elt).append(content2);                                     
			...
			switch(this.options.style)
			{
				case 'border':
					...
					if (currentRisk) {
						elt.parent().addClass(currentRisk);
					}
		...
		_applyEvents: function () {
			...
			// }, 300);
			$('.month-container').addClass("col-xs-12 col-sm-6 col-md-4");
		},
...
(function (jQuery, window, undefined) {
    ...
    var $doc = jQuery(document),
        ...
        Effects = { 
            MenuSearch : function() {
                //expande busca mobile
                jQuery(document).on('click', '.btn-toggle-search', function(e) {
                    jQuery('.box-search').toggleClass('open').fadeToggle('fast').find('.input-text').focus();
                });
            },
            MenuMobile : function() {
                //expande menu mobile
                jQuery(document).on('click', '.btn-toggle-menu', function(e) {
                    jQuery('.header-container-menu').toggleClass('open').fadeToggle('fast');
                    jQuery('body').toggleClass('body-menu-open');
                    //jQuery('.header').css('top', $win.scrollTop() );
                    jQuery('html,body').animate({ scrollTop: 0 }, 'slow');
                });
            },
            ExpandProfile : function() {
                //expande menu mobile
                jQuery(document).on('click', '.btn-toogle-profile', function(e) {
                    jQuery(this).parents('.profile-trigger').parent('.profile').toggleClass('open').find('.profile-container').slideToggle('fast');
                });
            },
            ToolTip : function() {
                jQuery(document).on('hover', '.tooltip-trigger', function(e) {
                    jQuery(this).find('.tooltip').fadeToggle('fast');
                });
            },
            DBRLogo : function() {
                if (window.innerWidth < 993) {
                    if (jQuery('.svg-dbr-mobo').length > 0) {
                        createDbrLogo('.svg-dbr-mobo', 'mouseover', themeFolder + '/images/svg/dbr-logo.svg');    
                    }
                } else {
                    if (jQuery('.svg-dbr-desk').length > 0) {
                        createDbrLogo('.svg-dbr-desk', 'mouseover', themeFolder + '/images/svg/dbr-logo.svg');    
                    }
                }
            },
            AdjustNavHeight : function() {
                var navContainer = jQuery('.container-nav'),
                windowHeight = window.innerHeight - 60, //barra top
                classHeight = windowHeight < 405 ? 'container-nav-scroll' : '';
                
                jQuery(navContainer).toggleClass(classHeight).innerHeight(windowHeight);
            },
            SantoTabs : function() {
                jQuery('.nav-tabs>li>a').click(function(e) {
                    ...
                    id = jQuery(this).attr("href").replace('#tab-',''),
                    tabContent = jQuery(this).parent().parent().siblings('.tab-content'),
                    _self = jQuery(this),
            ...
            ShowPassword : function() {
                jQuery('.show-password').click(function(e) {
                    jQuery(this).toggleClass('show-password-active').siblings('#user_pass').attr('type', function(index, attr){
                        return attr == 'password' ? 'text' : 'password';
                    });
                });
            },
            ExpandPrivacity : function() {
                jQuery('.acf-custom-publisher-privacity>.acf-label').click(function(){
                    jQuery(this).parent().toggleClass('acf-custom-publisher-privacity-open');
                    jQuery(this).next('.acf-input').toggle();
                })
                jQuery('html').click(function() {
                    jQuery('.acf-custom-publisher-privacity').removeClass('acf-custom-publisher-privacity-open');
                    jQuery('.acf-custom-publisher-privacity').find('.acf-input').hide();
                });
                jQuery('.acf-custom-publisher-privacity>.acf-label,.acf-custom-publisher-privacity>.acf-input').click(function(event){
                    event.stopPropagation();
                });
            },
        },
        ...
        FixTo = { 
            Init : function() {
                ...
                jQuery('.sticky').fixTo('body', options);
            }
        },

        Pages = { 
            Login : function() {
                var sectionMainHeight = window.innerHeight;
                jQuery('.page-login').find('.section-main').innerHeight(sectionMainHeight);

                if (jQuery('.svg-dbr-login').length > 0) {
                    createDbrLogo('.svg-dbr-login', 'mouseover', themeFolder + '/images/svg/dbr-logo.svg');    
                }
            },

            Content : function() {
                if (window.innerWidth > 991) {
                    var colLeft = jQuery('.box-page-content-left'),
                    colRight = jQuery('.box-page-content-right');
        ...
        JqueryUI = {
                ...
                if (jQuery('#search-input').length > 0) {
                    jQuery('#search-input').autocomplete({
                        ...
                        select: function( event, ui ) {
                            ...
                            //jQuery( "#result-id" ).val( ui.item.value ); // is this needed?
                            //jQuery( "#result-img" ).html( ui.item.img );
                            //jQuery( "#result-title" ).html( ui.item.label );
                            //jQuery( "#result-setor" ).html( ui.item.setor );
                            //jQuery( "#result-ramal" ).html( ui.item.ramal );
                            
                            return false;
                        }
                    }).data( "ui-autocomplete" )._renderItem = function( ul, item ) {
                        return jQuery( "<li class='autocomplete-result-search'>" )
                            .append(  
                                "<div class='person'>" + 
                                    "<div class='box-image'>" + item.img + "</div>" + 
                                    "<div class='box-content'>" + 
                                        "<h4 class='name'>" + item.label + "</h4>" + 
                                        "<h5 class='role'>" + item.setor + "</h5>" + 
                                        "<small class='extension'>" + item.ramal + "</small>" + 
                                    "</div>" + 
                                "</div>"
                            ).appendTo( ul );
            ...
            Select : function() {
                if (jQuery('.select-status-ferias').length > 0) {
                    jQuery('.select-status-ferias').selectmenu({
                        open: function( event, ui ) {
                            var source = '#' + jQuery(this).attr('id') + '-button',
                            target = '#' + jQuery(this).attr('id') + '-menu';

                            jQuery(target).width(jQuery(source).innerWidth());

                            return false;
                        },
                    });
                }
                if (jQuery('.select-status-materiais').length > 0) {
                    jQuery('.select-status-materiais').selectmenu({
                        open: function( event, ui ) {
                            var source = '#' + jQuery(this).attr('id') + '-button',
                            target = '#' + jQuery(this).attr('id') + '-menu';
                ...
                if (jQuery('.ui-spinner').length > 0) {
                    // jQuery('.ui-spinner').each(function(){
                    //   if(jQuery(this).spinner("instance")){
                    //     jQuery(this).spinner("destroy");
                    //   }else{
                        jQuery('.ui-spinner').spinner({min:0}).val(0);
                        // jQuery('.ui-spinner').spinner( "stepDown" );
                    //   }
                    // })
                }

                if (jQuery('.box-radio').length > 0) {
                    jQuery('.box-radio').controlgroup();
                }
            },

            // DatePicker : function(s, request) {
            ...
            HasMatch : function(s, request) {
                return s.toLowerCase().indexOf(request.term.toLowerCase())!==-1;
            },

            Lightwell : function(request, response) {
                var i, l, obj, matches = [];
                ...
                for  (i = 0, l = searchResults.length; i<l; i++) {
                    obj = searchResults[i];
                    if (JqueryUI.HasMatch(obj.label, request) || JqueryUI.HasMatch(obj.setor, request)) {
                        matches.push(obj);
                    }
                }
                response(matches);
            ...
            Accordion : function() {
                if (jQuery('.article-accordion').length > 0) {
                    jQuery('.article-accordion').accordion({
                        heightStyle: "content"
                    });    
                }
            },
        ...
        Comments = {
            Init : function() {
                jQuery(document).on('click', '.btn-trigger-comments', function(e) {
                    var articleComments = jQuery(this).parents('.article-footer');

                    if (jQuery(articleComments).find('.comments-hidden').is(':hidden')) {
                        jQuery(articleComments).find('.comments-hidden').slideToggle('fast');
                        jQuery(articleComments).toggleClass('article-footer-comments-open');
                    } else {
                        jQuery(articleComments).find('.comments-hidden').slideToggle('fast', function(){
                            jQuery(articleComments).toggleClass('article-footer-comments-open');
                        });
                    }
                });
        ...
        BrowserEvents = {
            Scroll : function() {
                $win.scroll(function (event) {
                    ...
                    windowInneHeight = $win.innerHeight();

                    if (windowInnerWidth > 1199) {
                        if (windowScrollTop > 100) {
                            $body.addClass('menu-floating');
                        } else {
                            $body.removeClass('menu-floating');
                        }
                    }
                });
        ...
        SlickSlider = {
            Init: function ($slider) {
                jQuery('.slider-alert-js, .wp_bannerize').slick({
                    arrows: true,
                    autoplay: true,
                    pauseOnHover: true,
                    // dots: true,
        ...
        DataTables = {
            Init: function () {
                ...
                jQuery('#table-escritorio-1-mobo').excelTableFilter({
                    columnSelector: '.apply-filter',
                    sort: true,
                    search: true,
                    captions: objLabels,
                });
                jQuery('#table-escritorio-1').excelTableFilter({
                    columnSelector: '.apply-filter',
                    sort: true,
                    search: true,
                    captions: objLabels,
                });
                jQuery('#table-escritorio-2').excelTableFilter({
                    columnSelector: '.apply-filter',
                    sort: true,
                    search: true,
                    captions: objLabels,
                });
                jQuery('#table-escritorio-2-mobo').excelTableFilter({
                    columnSelector: '.apply-filter',
                    sort: true,
                    search: true,
                    captions: objLabels,
                ...
                jQuery('.btn-expand').click(function(event) {
                    if (jQuery(this).parents('.ajax-item').hasClass('tr-expanded')) {
                        var selfParent = jQuery(this).parents('.ajax-item');

                        jQuery(this).parents('.ajax-item').next('tr').slideUp('fast', function(){
                            jQuery(selfParent).removeClass('tr-expanded');
                        });    
                    } else {
                        jQuery(this).parents('.ajax-item').addClass('tr-expanded').next('tr').slideDown('fast');    
                    }
        ...
        Calendar = {
            Init: function () {
                ...
                var currentYear = new Date().getFullYear();
                  jQuery.ajax({
                    ...
                    success: function(response) {
                      ...
                      if(response.success){
                        var events = response.data.events.map(function(item){
                         return {
                           id: item.id,
                           name: item.name,
                           startDate: new Date(moment(item.startDate).toDate()),
                           endDate: new Date(moment(item.startDate).toDate()),
                ...
                jQuery('input:checkbox.calFilter').on('change', function() {
                    var current = jQuery(this).val(),
                    eventTarget = '.day-' + current;
                    
                    // console.log('eventTarget', eventTarget);

                    jQuery('#calendar').find(eventTarget).toggleClass('day-unchecked');
                    jQuery(this).parents('.checkbox').toggleClass('checkbox-checked');
                });

                jQuery("#calendar").on('mouseenter', '.day-content', function() {
                    jQuery(this).find('.event-tooltip-wrapper').show();
                });
                jQuery("#calendar").on('mouseleave', '.day-content', function() {
                    jQuery(this).find('.event-tooltip-wrapper').hide();
                });
            },
        },
        ...
        Notification = {
            MockNotifications: {"success":true,"data":{"log_rows_raw":[],"log_rows":[{"id":"111","date":"2018-09-01 19:52:12","user_id":"1","permalink":"http:\/\/almap.santodev.com\/solicitacoes\/solicitacao-de-material-de-organictiger146-10\/","fullMessage":"&lt;span class=&#039;interpolate-text&#039;&gt;santopixel&lt;\/span&gt; fez uma nova solicita\u00e7\u00e3o.","message":"{display_name} fez uma nova solicita\u00e7\u00e3o."},{"id":"107","date":"2018-09-01 19:09:42","user_id":"","permalink":"","fullMessage":"Deleted <span class='interpolate-text'>feed item<\/span> \"<span class='interpolate-text'>Australian rent-a-grave law to be reviewed<\/span>\"","message":"Deleted {post_type} \"{post_title}\""},{"id":"106","date":"2018-09-01 19:09:42","user_id":"","permalink":"","fullMessage":"Deleted <span class='interpolate-text'>feed item<\/span> \"<span class='interpolate-text'>#worldcup &#8211; leo.bulhoes<\/span>\"","message":"Deleted {post_type} \"{post_title}\""},{"id":"105","date":"2018-09-01 19:09:41","user_id":"","permalink":"","fullMessage":"Deleted <span class='interpolate-text'>feed item<\/span> \"<span class='interpolate-text'>#worldcup &#8211; united_constant<\/span>\"","message":"Deleted {post_type} \"{post_title}\""},{"id":"104","date":"2018-09-01 19:09:41","user_id":"","permalink":"","fullMessage":"Deleted <span class='interpolate-text'>feed item<\/span> \"<span class='interpolate-text'>#worldcup &#8211; fm_cerulean<\/span>\"","message":"Deleted {post_type} \"{post_title}\""}],"num_new_rows":89}},
            
            Init: function(){
                setInterval(Notification.Watch, 10000);

                Notification.Effects();
                // Notification.Populate(Notification.MockNotifications);
            },

            /*
            @todo: Fazer um controle de clique e salvar a ultima data e hora que abriu as notificaçoes
            Passar essa data como parametro para filtrar a consulta
             */
            Watch: function () {
                var lastId = Cookies.get('notification') ? Cookies.get('notification') : null;

                jQuery.post({
                    url: ajax_url,
                    data: {
                      since_id : lastId, //catch last click on notification? last id notification
                      paged: 1, // fazer a navegação de notifications
                      action : 'notifications'
                    },
                    success: function(response){
                        if(response.success){
                            // Notification.Populate(response);
                            if('num_new_rows' in response.data && response.data.num_new_rows > 0){
                                jQuery('.btn-trigger-notifications').addClass('btn-notification-has-notification');
                            } else {
                                jQuery('.btn-trigger-notifications').removeClass('btn-notification-has-notification');
                            }
                        }
                    }
                });
            ...
            Fetch: function () {
                var boxNotifications = jQuery('#box-notifications');
                ...
                jQuery.post({
                    ...
                    beforeSend: function(response){
                        jQuery(boxNotifications).addClass('loading');
                    },
                    success: function(response){
                        console.log('fetch success', response);
                        if(response.success){
                            var lastId = response.data.log_rows[0].id ? response.data.log_rows[0].id : null;
                            
                            Notification.Populate(response);
                            Cookies.set('notification', lastId);
                        }
                    },
                    complete: function() {
                        jQuery(boxNotifications).removeClass('loading');
                    }
                });
            },
            
            Effects: function(data){
                var boxNotifications = jQuery('#box-notifications');

                jQuery('.btn-trigger-notifications').click(function(event) {
                    jQuery(this).removeClass('btn-notification-has-notification');

                    if(jQuery(boxNotifications).is(':hidden')) {
                        Notification.Fetch();
                        jQuery(boxNotifications).show().parent('.header-btns').addClass('open');
                    } else {
                        jQuery(boxNotifications).hide().parent('.header-btns').removeClass('open');
                    }
                });

                //click outside notications close itself
                jQuery('html').click(function(event) {
                    jQuery(boxNotifications).hide().parent('.header-btns').removeClass('open');
                });
                jQuery('.header-btns').click(function(event) {
                    event.stopPropagation();
                });
            },

            Populate: function(data){
                var rows = data.data.log_rows,
                html = '',
                rowHtml = [],
                boxNotificationsBody = jQuery('#box-notifications').find('.box-notifications-body');

                // console.log('index', i, 'val', val);
                // console.log('index', i, 'val', val.date);
                console.log('Populate', data);

                jQuery.each(rows, function(i, val) {
                    html +=  '<div class="notification-item notification-item-' + val.type + '" data-id="' + val.id + '">';
                    ...
                    html +=             Notification.HtmlDecode(val.fullMessage);;
                    html +=         '</div>';
                    html +=         '<div class="notification-date">';
                    html +=             jQuery.timeago(val.date);
                    html +=         '</div>';
                    html +=     '</div>';
                    html +=  '</div>';
                ...
                boxNotificationsBody.html(html);
            },
        ...
        AjaxFragment = {
            CheckHash : function() {
                ...
                // regExpBlog = new RegExp('^#\!?modal-feedback-([0-9]+)$', 'ig');

                if(hash != ''){
                  var link = jQuery('[data-src="'+hash+'"]');
        ...
        VPChecker = {
            Init: function () {
                jQuery('.article').viewportChecker({
                    classToAdd: 'article-visible',
                    classToAddForFullView: 'article-full-visible',
                    classToRemove: 'article-invisible',
                    ...
                    callbackFunction: function(elem, action){
                      ...
                      // if (action == 'add') {
                      //   console.log('elem: ', jQuery(elem).attr('id'), ' visible');
                      // } else {
                      //   console.log('elem: ', jQuery(elem).attr('id'), ' hidden');
                      // }
                    }
                });
            }
      ...
      InfiniteScroll = {
        Init: function () {
            var target = jQuery('.pagination-container').attr('data-container');

            if (target) {
                var inf = jQuery(target).infiniteScroll({
                    path: '.btn-more',
                    append: '.ajax-item',
    ...
    Almap.app = function () {
        return {
            init: function () {
                // console.log('Iniciou JS', 'tela', window.innerWidth + 'x' + window.innerHeight + 'x' + window.outerHeight);

                var isPageLogin = jQuery('body').hasClass('page-login'),
                isPageContent = jQuery('body').hasClass('page-template-template-conteudo'),
                isPageContentLive = jQuery('body').hasClass('page-template-template-conteudo-live');
                ...
                Effects.ExpandProfile();
                Effects.ToolTip();
                Effects.DBRLogo();
                Effects.SantoTabs();
                Effects.ShowPassword();
                Effects.ExpandPrivacity();
                InfiniteScroll.Init();
                ...
                if (isPageContent||isPageContentLive) {
                    Pages.Content();
                }
